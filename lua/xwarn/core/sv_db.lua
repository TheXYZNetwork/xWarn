...
function xWarn.Database.CreateWarn(userid, user, adminid, admin, reason)
	xAdmin.Database.Query(string.format("INSERT INTO %s_warns (userid, user, adminid, admin, reason, time) VALUES ('%s', '%s', '%s', '%s', '%s', '%s');", xAdmin.Info.Name, userid, xAdmin.Database.Escape(user) or "Unknown", adminid, xAdmin.Database.Escape(admin) or "Console", xAdmin.Database.Escape(reason) or "No reason given", os.time()))
end
